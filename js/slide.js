(function($){  // 매개변수(파라미터 Parameter)
    ...
    const obj = {  // 객체 리터럴 방식 권장
        ...
        section1(){
            ...
            // 1. 모든 슬라이드를 position absolute; 로 해준다
            // 2. 맨위 그리고 아래 포개진 슬라이드 앞뒤 z-index 3 2 1 순서로 정한다 
            // 3. 페이드 아웃 효과는 opacity 를 사용한다 시간운 0.6s or 1s
            // 4. 계속 10개의 슬라이드를 반복적으로 회전하여 순서 대로 보여 진다.
            mainSlide();
            function mainNextSlide(){
              slide.css({zIndex : 1, opacity:1});
              slide.eq(cnt).css({zIndex:2});
              slide.eq(cnt===0 ? n: cnt-1).css({zIndex:3}).stop().animate({opacity:0},1000);//cnt가 0이면 마지막슬라이드번호9번 그렇지 않다면 cnt-1 
              pageEvent();
            ...
            function mainPrevSlide(){
              ...
              slide.eq(cnt).css({zIndex:3}).stop().animate({opacity:0},0).animate({opacity:1},1000);//cnt가 0이면 마지막슬라이드번호9번 그렇지 않다면 cnt-1 
              pageEvent();
            ...
            function pageEvent(){
                ...
                pageBtn.eq( cnt>n-1 ? 0 : cnt).addClass('on');
            }
